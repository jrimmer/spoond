...
echo "=== 1. register rootfs (original; live sandbox is patched in step 6b) ==="
...
H64=$(echo "$HOOK" | base64 -w0)
curl -s -X POST "$API/v1/sandboxes/$SB/exec" -H "Content-Type: application/json" -d "{\"args\":[\"bash\",\"-c\",\"mkdir -p /etc/profile.d && echo $H64 | base64 -d > /etc/profile.d/forkd-tmux.sh && chmod +x /etc/profile.d/forkd-tmux.sh && grep -c exec /etc/profile.d/forkd-tmux.sh\"]}" | python3 -c "import sys,json; print('hook exec lines:', json.load(sys.stdin).get('stdout','').strip())"
